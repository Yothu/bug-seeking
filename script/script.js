...
for (let i = 0; i < projects.length; i += 1) {
  const projectId = projects[1].getAttribute('id');
  ...
  popupButtonContainer.add('popup-button-container');
...
function checkEmailUppercase() {
  ...
  for (let i = 0; i < contactEmailValue.length; i += 1) {
    if (contactEmailValue[i].match(/[a-s]/i)) {
